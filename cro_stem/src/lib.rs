...
// Normalizacija je izmještena u cro-normalize crate
...
lazy_static! {
    ...
    static ref VOICE_RULES: HashMap<&'static str, &'static str> = {
        ...
        map.insert("snj", "snijeg");
        map.insert("hrvatsk", "hrvatsk"); // Protect from suffix stripping
        map.insert("mislima", "misao");
        map.insert("šalic", "šalic"); // Protect šalic from -ice suffix
        map.insert("stručnj", "struč"); // Fix stručnjak -> struč
...
impl CroStem {
    ...
    pub fn stem_debug(&self, word: &str) -> Vec<String> {
        ...
        // 1. Hybrid Normalizacija (Dijakritici + Dijalekti) iz cro-normalize
        let normalized_word = cro_normalize::normalize(&current);
        if normalized_word != current {
            current = normalized_word.to_string();
...
mod tests {
    ...
    fn test_user_cases() {
        ...
        assert_eq!(stemmer.stem("strucnjak"), "struč");
        assert_eq!(stemmer.stem("salice"), "šalic");
        assert_eq!(stemmer.stem("case"), "čas");
        assert_eq!(stemmer.stem("objasnjava"), "objašnjav");
    }
}
...
pub fn normalize_wasm(word: &str) -> String {
    cro_normalize::normalize(word).to_string()
}
